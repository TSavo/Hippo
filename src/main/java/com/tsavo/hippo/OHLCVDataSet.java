...
public class OHLCVDataSet implements SortedSet<OHLCVData> {
	...
	public OHLCVDataSet(Set<Trade> someTrades, Duration aRollup) {
		DateTime nextDate = new DateTime(someTrades.iterator().next().getTimestamp().getTime()).withHourOfDay(0).withMinuteOfHour(0).withSecondOfMinute(0).withMillisOfSecond(0);
		do {
